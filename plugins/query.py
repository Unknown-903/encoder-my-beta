...
async def Cb_Handle(bot: Client, query: CallbackQuery):
    ...
    if data == 'home':
        btn = [
            ...
            [InlineKeyboardButton(text='📢 Uᴘᴅᴀᴛᴇs', url='https://t.me/Officialcreater'), InlineKeyboardButton
                (text='💻 Dᴇᴠᴇʟᴏᴘᴇʀ', url='https://t.me/Officialcreater')]
        ]
    ...
    if data.startswith('stats'):
        ...
        try:
            ...
            ffmpeg = "-preset superfast -c:v libx264 -vf scale=640:-2 -pix_fmt yuv420p -crf 30 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -vbr 2"
        ...
        try:
            c_thumb = await db.get_thumbnail(query.from_user.id)
            ffmpeg = "-preset superfast -c:v libx264 -vf scale=1280:-2 -pix_fmt yuv420p -crf 30 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -vbr 2"
